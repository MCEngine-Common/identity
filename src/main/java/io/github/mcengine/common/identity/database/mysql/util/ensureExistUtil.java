...
/**
 * Ensures the player's identity base rows exist (MySQL).
    ...
    public static boolean invoke(Connection conn, Plugin plugin, Player player) {
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(
                    ...
                    "VALUES (?,?,?,?,?,?) " +
                    "ON DUPLICATE KEY UPDATE identity_alternative_updated_at=VALUES(identity_alternative_updated_at)")) {
            ...
            plugin.getLogger().warning("ensureExistUtil failed: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
